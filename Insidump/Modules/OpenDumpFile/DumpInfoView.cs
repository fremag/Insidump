...
public class DumpInfoView : ViewBase
{
    public DumpInfoView(DumpInfo dumpInfo)
    {
        ...
        var segmentSizeInfos = dumpInfo.SegmentSizesMo
            .GroupBy(sizeMo => sizeMo )
            .OrderByDescending(groupSize => groupSize.Key)
            .Select(groupSize => $"{groupSize.Key} Mo x {groupSize.Count()}");
        
        var segmentsMo = string.Join(", ",segmentSizeInfos);
        var lblSegments = new Label { Y = Pos.Bottom(lblNbSegments),        Text = $"Segments (Mo x N):  {segmentsMo}", Width = Dim.Fill() };
