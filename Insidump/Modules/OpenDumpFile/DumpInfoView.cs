...
public class DumpInfoView : ViewBase
{
    public DumpInfoView(DumpInfo dumpInfo)
    {
        ...
        var lblImgSize = new Label { Y = Pos.Bottom(lblFlavor),             Text = $"ImageSize:      {dumpInfo.ModuleInfo.ImageSize}", Width = Dim.Fill() };
        ...
        var lblSegments = new Label { Y = Pos.Bottom(lblNbSegments),        Text = $"Segments (Mo):  {string.Join(", ",dumpInfo.Segments)}", Width = Dim.Fill() };
